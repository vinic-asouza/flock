...
4. `authMiddleware` valida via `supabase.auth.getUser(token)` e anexa `req.user` + `req.church`  
   Rotas `/api/ops/*` autenticadas usam `authUserOnly` + `requirePlatformAdmin` (allowlist, **sem** `attachChurchContext`). GETs de Igrejas são read-only e **não** devolvem PII de Membros (BR-OPS-005/006). `GET /api/ops/health` usa a **sessão** do operador — **não** `INTERNAL_BILLING_TOKEN` / `HEALTH_CHECK_TOKEN` / `METRICS_TOKEN` (BR-OPS-007).
...
| Front | `canEdit = currentRole !== 'reader'`; `canCreateCongregations` / selects limitados ao escopo |
| DTO | `sanitizeChurchForRole` remove campos Stripe de respostas para não-admin |

**ABAC leve (intra-tenant):** além de role + `church_id`, `reader`/`editor` podem ser limitados a um subconjunto de congregações (`access_all_congregations` ou N:N `church_user_congregations`). Owner/admin têm acesso total dinâmico. Enforcement **aplicacional** (service_role); nunca confiar só no frontend.
