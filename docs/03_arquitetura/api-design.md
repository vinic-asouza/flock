...
| GET | `/api/ops/waitlist` | 👤 + `requirePlatformAdmin` | — | Lista paginada de leads (RL 60/15min; default `status=pending`); **não** existe `GET /api/waitlist` | ✅ |
| PATCH | `/api/ops/waitlist/:id` | 👤 + `requirePlatformAdmin` | — | Situação `converted` \| `discarded` a partir de `pending` (RL 60/15min); 404/409; não cria Igreja | ✅ |
...
| POST | `/api/waitlist/` | ❌ | — | Lead landing (público). Operação autenticada: `GET`/`PATCH /api/ops/waitlist` | ✅ |
