...
> Programas → Turmas → Inscritos → Aulas/Chamada → Materiais, com match N/W/D ao rol, fila de possível membro e link público da turma.  
...
Organiza ciclos formativos da igreja (**EBD**, cursos, estudos, treinamentos): catálogo de **Programas**, **Turmas** por congregação, **matrículas** (membro, convidado ou possível membro), **cronograma de aulas** (avulsas e séries), **chamada** por encontro e **materiais** da turma (links e anotações para a equipe).
...
- PDF de chamada, capacidade/lista de espera, LMS, upload de arquivos, materiais por aula, área do aluno
- Certificados (Issue filha)
...
backend/src/
├── routes/
│   └── teaching.ts                 → programs / classes / lessons / attendance / enrollments / materials / public-link
├── controllers/
...
└── components/teaching/            → UI, filtros, detalhe (abas), inscritos, aulas/chamada, materiais, modais

Testes: match, enrollment policy, validators (incl. materiais/URL), recurrence, attendance eligibility
...
Referências gerais da turma (não por aula). `type`: `link` \| `note`. Link exige `url` (http/https) e `content` null; anotação exige `content` e `url` null. Título 2–120. Ordenação de lista: `updated_at DESC`. CASCADE com a turma. Sem upload de arquivo.

**Soft delete:** matrícula usa `removed_at` (histórico de chamada preservado); aulas/séries usam hard delete via RPC de escopo; materiais usam hard delete.  
**Auditoria:** timestamps; mutações sensíveis via `audit_logs` (entidades de aula/série/presença/material).
...
| PATCH/DELETE | `/api/teaching/materials/:materialId` | editor | Atualiza (sem mudar `type`) / exclui |
...
UI: matrículas e fila ficam na aba **Inscritos** (termo de negócio **Aluno** = matrícula; ver glossário). Aba **Aulas** cobre calendário/lista, detalhe e chamada (`?tab=aulas&lessonId=`). Aba **Materiais** cobre links e anotações da turma (`TeachingMaterialsTab`). Certificados permanecem placeholder (DEV-110).
...
- Código EN (`teaching_*`); UI PT (**Ensino / Programa / Turma / Inscritos / Aulas / Chamada / Materiais**).
...
- Materiais são da **turma** (não da aula); IDs de regra: BR-ENS-024/025 (019–023 já cobrem aulas/chamada).
