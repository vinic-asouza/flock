...
Avanço entre etapas (`Todo` → `In Progress` → `Review` → `Document` → `Done`) exige **comando no chat** (Gate de Avanço, workflow §15.2), exceto `Backlog` → `Todo` após Product Analyst e Software Architect. O PR da Issue abre em `In Progress` (§15.6). Ao autorizar `Done`, o agente pergunta se deseja **mergear** o PR (§15.3).
