...
- **Usado em:** Membros, Integração, Ministérios, Calendário, Relatórios  
...
- **Nota:** API e tabelas permanecem `groups` / `Group` / `member_groups`. Coluna `type` / enum `GroupType` **removidos** (DEV-115). Substitui “cargos” e o hub legado “Grupos”.
...
> Lead captado na landing antes/fora do funil completo de registro. Captação pública: `POST /api/waitlist`. Operação no Admin OPS `/waitlist`: `GET /api/ops/waitlist` + `PATCH` de situação (`pending` / `converted` / `discarded`). **Não** há GET na rota pública. Excluir **não** apaga a linha nem libera o e-mail (BR-ACQ-003).
...
> Centro operacional interno do SaaS. Superfície própria (local `:3002`), usada pelo **Operador da plataforma**. **Não** é o Painel da Igreja. Auth: `/api/ops` + allowlist + login/shell. Console read-only: overview, lista e ficha de Igrejas. **Lista de espera** (`/waitlist`): leads da landing, com marcar convertido/excluído. Tela **Saúde** (`/health`): status de API, Stripe e jobs de billing. Sem documentação Mintlify (ferramenta interna).
...
- **C:** Calendário (item), Checkout, Cliente comercialmente ativo, Congregação, Conta, Converter, CNPJ  
...
- **G:** Guest/Convidado (calendário); Grupo (termo de código/`groups` — preferir **Ministério** na UI)  
...
- **M:** Membro, Mentor, Membership, Ministério  
